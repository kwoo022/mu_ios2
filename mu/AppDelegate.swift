...
//  nhu
//
//  Created by idino on 2023/06/22.
//
...
extension AppDelegate : UNUserNotificationCenterDelegate {
    ...
    public func userNotificationCenter(_ center: UNUserNotificationCenter,
        ...
        if response.actionIdentifier == UNNotificationDefaultActionIdentifier{
            let next = PushView(TGSPushSceneModel(isShowSelf: .constant(true), command: TGSMCommad())).environmentObject(TGSAppInfo(isShowSelf: .constant(true))).navigationViewStyle(StackNavigationViewStyle())
            let nextViewController = UIHostingController(rootView: next)
...
struct PushView: View{
    // MARK: PROPERTIES
    @State var viewModel : TGSPushSceneModel
    ...
    func movePushScene(){
        let viewModel = TGSPushSceneModel(isShowSelf: self.coordinator.getTriggerBinding(), command: TGSMCommad())
